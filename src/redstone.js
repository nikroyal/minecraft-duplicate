...
export function updateRedstoneNetworkAround(centerX, centerY, centerZ, radius = 16) {
  ...
  while (queue.length > 0) {
    ...
    for (const dir of CARDINAL_DIRECTIONS) {
      ...
      if (targetId === 60 || targetId === 61 || targetId === 63 || targetId === 64 || targetId === 70 || targetId === 71 || targetId === 56 || targetId === 117 || targetId === 118 || targetId === 119 || targetId === 72 || targetId === 73) {
  ...
  for (let x = startX; x <= endX; x++) {
    for (let y = startY; y <= endY; y++) {
      for (let z = startZ; z <= endZ; z++) {
        ...
        // TNT (ID 56, 117, 118, 119) ignition
        if ((id === 56 || id === 117 || id === 118 || id === 119) && p > 0) {
